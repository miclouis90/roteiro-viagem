...
export function filterHomeTrips(trips: readonly Trip[], query: string, status: TripStatus | "", today = dateKey()) {
  const term = normalize(query);
  const filtered = trips.filter(t => (!status || t.status === status) && normalize(`${t.title} ${t.destinationCity} ${t.destinationState} ${t.country}`).includes(term));
